...
<html>
	<script>
		...
		function textCheck(){
			var textareaStr = document.getElementById("info").value;
			var titleBoxStr = document.getElementById("name").value;
			document.getElementById("info").value = textareaStr.replaceAll("&", "&amp;").replaceAll("<", "&lt;").replaceAll(">", "&gt;").replaceAll("\"", "&quot;").replaceAll("\\", "&#39;");
		...
		<form method="post">
			<table>
				<tbody>
					...
					<tr><td>상품명</td>
						<td><input type="text" name="name" id="name" required></td></tr> <!-- 이름 필수 입력 사항 -->
					<tr><td>재고현황</td>
						<td><input type="number" min=0 name="count" required></td></tr><!-- 재고 현황 입력 폼 -->
					<tr><td>상품 등록일</td><td><%=sdf.format(now) %></td></tr> <!-- 날짜는 오늘 날짜로 초기화 -->
					<tr><td>재고 등록일</td><td><%=sdf.format(now) %></td></tr> <!-- 날짜는 오늘 날짜로 초기화 -->
					<tr><td>상품 설명</td>
						<td><textarea style="width:300px; height:100px;"name="info" id="info" required></textarea></td></tr> <!--  내용은 textarea로 받기 -->
